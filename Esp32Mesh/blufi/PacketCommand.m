...
+(NSMutableData *)SetMeshID:(NSData *)MeshID Sequence:(uint8_t)sequence Encrypt:(BOOL)Isencrypt WithKeyData:(NSData *)keydata
{
    uint8_t dataByte[4];
    dataByte[0]=(0x09<<2) | 0x01;              //数据包，设置meshid
    if(Isencrypt)
    {
    ...
    if (Isencrypt) {
        ...
        MeshID=[DH_AES blufi_aes_Encrypt:dataByte[2] data:byte len:(int)MeshID.length KeyData:keydata];
    }
    //拼接
